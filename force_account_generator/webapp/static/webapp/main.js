...
const appData = {
  ...
  runDemo() {
    ...
    // dropzone.enqueueFile(new File(['this is a demo'], 'demo.xlsx'));
    const mockFile = { name: 'demo.xlsx', size: 1500000 };
    ...
    const uploader = setInterval(() => {
      this.bytesSent += 50000;
      if (this.bytesSent >= mockFile.size) {
        clearInterval(uploader);
        sendDemoRequest(mockFile);
      }
    }, 100);
  },
};
...
function sendDemoRequest(mockFile) {
  fetch('/demo')
    .then((res) => res.json())
    .then((data) => {
      dropzone.emit('success', mockFile, data);
    });
}
