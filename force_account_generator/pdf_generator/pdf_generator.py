...
def generate_pdf(input_data, output, daily_sheets=False, callback=None):
    config = PdfFieldConfig()
    data_loader = DataLoader(input_data)
    pdf = PdfPackage(data_loader, config, output, daily_sheets=daily_sheets, callback=callback)
