...
class InputData:
    def __init__(self, json_config_file):
        ...
        self.material = __load_material(data["material"])
        self.labor = __load_labor(data["labor"])
        self.equipment = __load_equipment(data["equipment"])
        self.rentals = __load_rentals(data["rentals"])
        self.services = __load_services(data["services"])
        self.purchased_consumables = __load_purchased_consumbles(
            data["purchased_consumables"])
        self.stock_consumables = __load_stock_consumables(
            data["stock_consumables"])


def __load_material(data):
    material_list = []
...
def __load_labor(data):
    ...
    for labor_data in data["data"]:
        ...
        for day in labor_data["daily_hours"]:
            labor.add_daily_hours(day["date"], day["st"], day["ot"])
...
def __load_equipment(data):
    ...
    for equip_data in data["data"]:
        ...
        for day in equip_data["daily_hours"]:
            equip.add_daily_hours(day["data"], day["op"], day["sb"])
...
def __load_purchased_consumbles(data):
    consumable_list = []
...
def __load_stock_consumables(data):
    consumable_list = []
    for consumable_data in data["data"]:
